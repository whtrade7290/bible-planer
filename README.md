...
`PORT` 변수를 지정하지 않으면 기본값은 `8080`입니다.
...
## API 사용법
- **GET `/bible`**: 간단한 안내 문구만 반환합니다.
- **POST `/bible`**: 본 기능. JSON 본문에 `days` 값을 넣어 요청하면 요청 일수에 맞춰 `성경통독표(<일수>일).csv` 파일을 생성하고 곧바로 다운로드 응답으로 전송합니다.
...
- CSV는 루트 디렉터리에 저장되며, 동일한 이름의 파일이 있으면 덮어씁니다.
